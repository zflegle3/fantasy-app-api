...
exports.chat_get_id = asyncHandler(async (req, res) => {
    ...
    let messagesLimit =  chat.messages.length > 10 ? chat.messages.slice(-10) : chat.messages;
    if (chat) {
        res.json({
            name: chat.name,
        ...
        res.status(400)
        throw new Error("Invalid data, chat data not found")
    }
